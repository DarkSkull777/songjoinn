...
DELETE_DELAY = 2880
DURATION_AUTOPLAY_MIN = 1200
DURATION_PLAY_HOUR = 20
...
USERBOT_REPO = f"""{emoji.ROBOT} **Obrolan Suara Userbot Telegram**
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await mp.group_call.client.send_message(
            "me",
            f"{emoji.CHECK_MARK_BUTTON} Bergabung ke obrolan suara"
        )
    else:
        mp.chat_id = None
        await mp.group_call.client.send_message(
            "me",
            f"{emoji.CROSS_MARK_BUTTON} Keluar dari obrolan suara"
        )
...
async def join_voice_chat(client, m: Message):
    ...
    if 2 <= len_command <= 4:
        ...
        if group_call.is_connected:
            text = f"{emoji.ROBOT} Sudah bergabung ke obrolan suara"
        else:
            await group_call.start(channel, join_as=join_as,
                                   invite_hash=invite_hash)
            # text = "Status akan dikirim ke Pesan Tersimpan"
            return
    else:
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        await m.reply_text(
            f"{emoji.MUSICAL_NOTES} **Saat ini bot bergabung di grup**:\n"
            f"- **{chat.title}**",
            quote=True
        )
    else:
        await m.reply_text(f"{emoji.NO_ENTRY} Saat ini bot tidak bergabung dengan obrolan suara manapun.t",
                           quote=True)
...
async def play_track(client, m: Message):
    ...
    if m.audio:
        ...
        if m_audio.audio.duration > (DURATION_PLAY_HOUR * 1200 * 1200):
            reply = await m.reply_text(
    ...
    if playlist and playlist[-1].audio.file_unique_id \
        ...
        reply = await m.reply_text(f"{emoji.ROBOT} Telah di tambahkan ke playlist", quote=True)
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} Bentar cok lagi di proses...",
            quote=True
        )
...
async def mute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.MUTED_SPEAKER} Telah di mute!", quote=True)
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(
        f"{emoji.SPEAKER_MEDIUM_VOLUME} Telah di unmute!",
        quote=True
    )
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def skip_current_playing():
    ...
    print(f"- Sekarang sedang play: {playlist[0].audio.title}")
    await mp.send_playlist()
